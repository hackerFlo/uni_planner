...
export default function DayColumn({ date, todos, onUnassign, onComplete, onEdit }) {
  ...
  return (
    ...
    >
      <div className={`px-3 py-3 border-b ${isToday ? 'border-indigo-100' : 'border-zinc-100'}`}>
        <div className="flex items-baseline gap-1.5">
          <span className={`text-xs font-semibold uppercase tracking-widest ${isToday ? 'text-indigo-500' : 'text-zinc-400'}`}>
            {dayLabel}
          </span>
          {isToday && (
        ...
        <p className={`text-lg font-light mt-0.5 ${isToday ? 'text-indigo-700' : 'text-zinc-700'}`}>
          {dayNum} <span className="text-sm text-zinc-400">{month}</span>
        </p>
      </div>

      <div
        className={`flex-1 p-2.5 space-y-2 min-h-[120px] transition-colors rounded-b-xl ${
